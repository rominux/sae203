...
image::image/selection.png[Sélection des logiciels, pdfwidth=60%, width=60%]
...
----
man usermod footnote:[https://manpages.debian.org/trixie/passwd/usermod.8.en.html[Page de manuel usermod]]
...
----
man mount footnote:[https://manpages.debian.org/trixie/mount/mount.8.en.html[Page de manuel mount]]
...
. **Dernier nom de code annoncé** : _À l'heure actuelle, la version 14 est `Forky` et la version 15 sera `Mac`. Forky a été annoncé pour faire suite à Trixie._
...
Nous avons modifié le paramètre par défaut :
[source,plaintext]
----
d-i passwd/user-default-groups string audio cdrom video
----
Pour y rajouter `sudo` :
[source,plaintext]
----
...
La commande `git remote -v` permet de lister ceux qui sont configurés sur votre projet.
...
`git pull` : effectue un *fetch* puis fusionne directement ces nouvelles données avec votre branche locale actuelle.
|===
...
| Il faut générer une paire de clés SSH sur l'ordinateur avec la commande `ssh-keygen -t ed25519 -C "exemple@univ-lille.fr"`, puis copier le contenu de la clé publique générée pour l'ajouter dans les paramètres de sécurité de votre compte sur la forge logicielle.
|===
...
| La commande `git init` crée un sous-dossier `.git` caché qui initialise la structure d'un dépôt vide dans le répertoire actuel.
...
git log
----
image::image/log.png[Résultat du git log, pdfwidth=100%, width=100%]

| **Créez une nouvelle branche, faites des modifications, puis fusionnez (merge) cette branche avec la principale. Expliquez.**
a| 
image::image/branche.png[Git branche, pdfwidth=100%, width=100%] 
Les branches permettent de développer une fonctionnalité isolée ou de corriger un bug en parallèle, puis de l'intégrer au code principal (`main` ou `master`) une fois l'opération terminée et validée.

| **Qu'est-ce qu'un conflit de fusion ? Provoquez-en un et montrez comment le résoudre.**
a| 
image::image/conflit_terminal.png[Conflit terminal, pdfwidth=100%, width=100%]
image::image/conflit_nano.png[Conflit nano, pdfwidth=40%, width=40%]
Un conflit survient lorsque deux branches modifient la même ligne d'un fichier de façon différente. Lors de la fusion, Git met le processus en pause et demande une résolution manuelle (choisir le code à garder, supprimer les marqueurs de conflit puis effectuer un commit final).

| **Utilisez git diff pour comparer deux commits. Expliquez la sortie.**
a| 
image::image/diff_terminal.png[Diff terminal, pdfwidth=40%, width=40%]
Les lignes précédées d'un `-` en rouge sont des suppressions et celles avec un `+` en vert sont des ajouts. Les autres lignes affichées servent de contexte autour du code modifié.

| **Qu'est-ce que le fichier .gitignore ? Créez-en un pour ignorer les fichiers Java (.class, .jar).**
a| 
C'est un fichier texte permettant de lister les expressions et motifs des fichiers/dossiers qui ne doivent pas être suivis et versionnés par Git.
[source,plaintext]
----
*.class
*.jar
