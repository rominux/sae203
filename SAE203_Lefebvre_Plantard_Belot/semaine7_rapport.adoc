...
=== 1. Création de la machine virtuelle dans VirtualBox
...
=== 2. Installation de l'OS de base (Debian 13)
...
. **Le miroir réseau** : Nous avons utilisé le miroir local de l'université `http://debian.polytech-lille.fr` pour accélérer le téléchargement des paquets.
. **Les utilisateurs** : 
  * Un mot de passe robuste a été défini pour l'administrateur (`root`).
  * Un compte utilisateur standard nommé `user` a été créé pour l'utilisation quotidienne.
...
=== 3. Configuration post-installation et Suppléments invités

Une fois le système installé et redémarré, deux configurations manuelles ont été nécessaires pour rendre la machine parfaitement utilisable :
...
=== 4. Automatisation de l'installation (Preseed)
...
=== 5. Retour d'expérience et difficultés rencontrées
