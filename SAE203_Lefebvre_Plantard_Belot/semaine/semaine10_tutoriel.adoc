...
Pour travailler en équipe, nous avons utilisé un dépôt distant sur GitLab. Nous avons d'abord généré une clé SSH footnote:[Secure Shell : un protocole cryptographique permettant de sécuriser les communications (ici, l'authentification Git) sur un réseau non sécurisé.] (`ssh-keygen -t ed25519 -C "exemple@univ-lille.fr"`) que nous avons ajoutée à notre profil GitLab pour l'authentification.
...
Bien que la ligne de commande soit rapide, nous avons testé une interface graphique web nommée **Ungit**. Après avoir installé Node.js footnote:[Un environnement d'exécution JavaScript côté serveur.] et `npm` footnote:[Node Package Manager : le gestionnaire de paquets officiel pour l'écosystème Node.js.] sur le système, nous l'avons installé via la commande globale : `sudo -H npm install -g ungit`. Ensuite, il se lance en tapant `ungit` et s'ouvre directement sous forme d'onglet dans le navigateur web.
