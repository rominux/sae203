...
* **Pourquoi avez-vous choisi ce logiciel ?** +
  Nous avons choisi **Ungit** pour son approche extrêmement visuelle. Il représente l'historique de Git sous forme de nœuds interactifs et d'animations très intuitives, ce qui rend la compréhension des branches beaucoup plus claire que les vieux clients natifs footnote:[https://github.com/FredrikNoren/ungit[Dépôt officiel GitHub de Ungit]].
* **Comment l'avez-vous installé ?** +
  Ungit est un paquet Node.js. Après avoir installé Node.js et `npm` sur le système, nous l'avons installé via la commande globale : `sudo npm install -g ungit`. Ensuite, il se lance en tapant `ungit` et s'ouvre directement sous forme d'onglet dans le navigateur web.
