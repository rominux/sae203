...
| On a fait des merge requests dans gitlab pour fusionner les branches. Nous avons pas eu de conflits car nous avons pas modifié les meme fichiers.

| **Qu'est-ce qu'une pull request (ou merge request) ? À quoi sert-elle ?**
| Une pull/merge request sert a fusionner une branche source vers une branche destination.
